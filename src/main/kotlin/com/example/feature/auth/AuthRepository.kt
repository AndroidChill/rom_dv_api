...
import com.example.statuspages.UserAlreadyExist
...
class AuthRepository : KoinComponent {
    ...
    suspend fun getUserByPhoneOrEmail(phone: String, email: String): UserFull? = dbQuery {
        UserTable.select { (UserTable.phone eq phone) or (UserTable.email eq email) }
    }.mapNotNull { row ->
        row.toFullUser()
    }.singleOrNull()
